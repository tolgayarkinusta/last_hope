...
def yellowOnly(frame, depth, center_x, center_y, green_detected, red_detected, yellow_detected, blue_detected, black_detected, yellow_positions):
    if yellow_detected and not (green_detected or red_detected or blue_detected or black_detected):
        ...
        # Bekleme süresi kaldırıldı, direkt geri hareket:
        controller.set_servo(5, 1450)
        controller.set_servo(6, 1450)
    else:
    ...
    if red_detected and green_detected and not (yellow_detected or blue_detected or black_detected):
        ...
        if depth_diff > 5:
            ...
            if closest_green_depth > closest_red_depth:
                print("Derinlik farkı > 5m, yeşil derinlik > kırmızı: redOnly çağrılıyor.")
                ...
                print("Derinlik farkı > 5m, yeşil derinlik <= kırmızı: greenOnly çağrılıyor.")
    ...
    if yellow_detected and green_detected and not (red_detected or blue_detected or black_detected):
        ...
        if depth_diff > 5:
            ...
            if closest_yellow_depth > closest_green_depth:
                print("Derinlik farkı > 5m, sarı derinlik > yeşil: greenOnly çağrılıyor.")
                greenOnlyRecursive(frame, depth, center_x, center_y, green_detected, red_detected, yellow_detected, blue_detected, black_detected, green_positions)
            else:
                print("Derinlik farkı > 5m, sarı derinlik <= yeşil: yellowOnly çağrılıyor.")
    ...
    if yellow_detected and red_detected and not (green_detected or blue_detected or black_detected):
        ...
        if depth_diff > 5:
            ...
            if closest_yellow_depth > closest_red_depth:
                print("Derinlik farkı > 5m, sarı derinlik kırmızıdan büyük: redOnly çağrılıyor.")
                redOnlyRecursive(frame, depth, center_x, center_y, green_detected, red_detected, yellow_detected, blue_detected, black_detected, red_positions)

            else:
                print("Derinlik farkı > 5m, sarı derinlik kırmızıdan küçük veya eşit: yellowOnly çağrılıyor.")
                yellowOnlyRecursive(frame, depth, center_x, center_y, green_detected, red_detected, yellow_detected, blue_detected, black_detected, yellow_positions)
...
def yellowOnlyRecursive(frame, depth, center_x, center_y, green_detected, red_detected, yellow_detected, blue_detected,
    ...
    #(frame, "Going Astern!!", (center_x - 50, center_y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2)
...
def main():
    ...
    # Global değişkenler: video kaydı için
    recording = False
    video_writer = None
    ...
    while True:
        ...
        if zed.grab() == sl.ERROR_CODE.SUCCESS:
            ...
            #results = model(frame, conf=0.40)[0]
            ...
            if key == ord('m'):
                ...
                time.sleep(0.2)
            # --- EK: Video kayıt kontrolü ---
            # 'y' tuşuna basılırsa kayıt başlasın, 'u' tuşuna basılırsa kayıt bitsin ve .mp4 formatında kaydedilsin.
            if key == ord('y'):
                if not recording:
                    video_filename = "SEKILVideo.mp4"
                    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
                    # Kaydedilecek video boyutu, frame'in orijinal boyutu
                    video_writer = cv2.VideoWriter(video_filename, fourcc, 30.0, (frame.shape[1], frame.shape[0]))
                    recording = True
            ...
            if key == ord('u'):
                if recording:
                    ...
                    recording = False
                    print("Recording stopped, saved as output.mp4")
            ...
            # Eğer video kaydı aktifse, frame'i dosyaya yaz
            if recording:
        ...
        # Eğer kayıt hâlâ aktifse, çıkmadan video writer'ı serbest bırak
