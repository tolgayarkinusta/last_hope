...
def initialize_positional_tracking(zed):
    ...
    Raises:
        SystemExit: Pozisyon takibi başlatılamazsa programı sonlandırır
    """
...
def find_closest_object(positions, depth):
    """
    Verilen nesne pozisyonları arasından en yakın olanını bulur.
    
    Args:
        positions: Nesne sınırlayıcı kutularının koordinatlarını içeren liste [x1, y1, x2, y2]
        depth: Derinlik verisi içeren ZED kamera derinlik matrisi
        
    Returns:
        closest_depth: En yakın nesnenin derinlik değeri (metre)
        closest_center: En yakın nesnenin merkez koordinatları (x, y)
    """
    closest_depth = float('inf')
    closest_center = None
