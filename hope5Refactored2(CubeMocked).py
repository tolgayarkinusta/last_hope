...
scale = 60        # 1 metre = 50 piksel (uyarlamayı ihtiyaç duyarsan değiştirebilirsin)
...
class USVController:    #todo: mock hope6da düzeltilsin, hope6 beta test kodu olacak
    def __init__(self):
        print("mocking")
...
def initialize_camera():
    ...
    init_params.coordinate_system = sl.COORDINATE_SYSTEM.RIGHT_HANDED_Z_UP  # default for the opencv
...
def main():
    ...
    while True:
        ...
        if zed.grab() == sl.ERROR_CODE.SUCCESS:
            ...
            # Check if the data has been updated since the last time. Retrieve the current sensors sensors_data
            if zed.get_sensors_data(sensors_data,
                ...
                if ts_handler.is_new(sensors_data.get_imu_data()):
                    ...
                    # todo: accuracy yüzdelik mi ondalık ona göre format ayarlaması yapılacak
                    )
            ...
            if manual_mode:
                ...
                if yellow_detected and red_detected and green_detected and not (blue_detected or black_detected):
                    greenRedYellowOnly(frame, depth, center_x, center_y, green_detected, red_detected, yellow_detected,
                                       blue_detected, black_detected, green_positions, red_positions, yellow_positions)
            ...
            cv2.putText(frame, f"{str(zed.get_spatial_mapping_state())}", (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (20, 220, 20), 1)
            cv2.putText(frame, f"POSITIONAL_TRACKING_STATE.{str(zed.get_position(pose, sl.REFERENCE_FRAME.WORLD))}",(10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (20, 220, 20), 1)
            cv2.putText(frame, f"Coordinates X,Y: {current_x:.1f} {current_y:.1f}",(10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 150, 240), 1,)
