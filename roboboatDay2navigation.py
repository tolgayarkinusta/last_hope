...
def docking(frame, depth, current_x, current_y, magnetic_heading, center_x, center_y, map_image,
    ...
    if docking.phase == "compute_line":
        ...
        if len(shape_world_points) < 2:
            ...
            #todo: yarışma haritasına göre ilk görevden sonra docking yoksa gezinme/arama algoritması eklenebilir
            return False
...
def special_mission(frame, depth, current_x, current_y, magnetic_heading, center_x, center_y, map_image, detections):
    ...
    if not hasattr(special_mission, "phase"):
        ...
        # Tespit zamanını izlemek için:
        special_mission.last_seen = time.time()

    # --- Engel Kontrolü ve Görselleştirme ---
    # Hareket gerektiren aşamalarda, ekran orta noktasının 40 piksel üstündeki yatay çizgide,
    # 20 piksel aralıklarla derinlik kontrolü yapılır. Her kontrol noktası, derinliğe göre renklenir.
    if special_mission.phase in ["approach", "maintain", "return", "rotate"]:
        obstacle_y = center_y - 40  # ekran orta noktasının 40 piksel üstü
        frame_width = frame.shape[1]
        min_depth = float('inf')
        min_x = None
        for x in range(0, frame_width, 20):  # 20 piksel aralık
            d = depth.get_value(x, obstacle_y)[1]
            # Görselleştirme: 0.5 m'nin altındaki noktalar kırmızı, aksi halde yeşil
            color = (0, 0, 255) if d < 0.5 else (0, 255, 0)
            cv2.circle(frame, (x, obstacle_y), 3, color, -1)
            if d < min_depth:
                min_depth = d
                min_x = x
        if min_depth < 0.5:
            if min_x < center_x:
                cv2.putText(frame, "Engel tespit: sol, sag donus", (50, 500), FONT, 1, (0, 0, 255), 2)
                correction = 50  # sabit dönüş hızı değeri
                controller.set_servo(5, 1530 + correction)
                controller.set_servo(6, 1470 - correction)
            else:
                cv2.putText(frame, "Engel tespit: sag, sol donus", (50, 500), FONT, 1, (0, 0, 255), 2)
                correction = 50
                controller.set_servo(5, 1470 - correction)
                controller.set_servo(6, 1530 + correction)
            return False

    # --- Special Object Sürekliliğinin Sağlanması ---
    # "init" dışındaki aşamalarda, her frame'de özel nesnenin tespiti kontrol edilir.
    # Eğer tespit edilirse bounding box (koordinatlar) güncellenir ve son görülme zamanı yenilenir.
    # Eğer 2 saniyeden fazla tespit sağlanmazsa veya nesnenin derinliği 5 metreden fazlaysa,
    # otomatik olarak "rotate" aşamasına geçilir.
    if special_mission.phase not in ["init", "rotate"]:
        mission_class = 2 if special_mission.mission_type == 40 else 5
        found = False
        for i, class_id in enumerate(detections.class_id.tolist()):
            if class_id == mission_class:
                special_mission.object_box = detections.xyxy.tolist()[i]
                special_mission.last_seen = time.time()
                found = True
                break
        if found:
            # Tespit edilen nesnenin derinliğini kontrol et
            x1, y1, x2, y2 = map(int, special_mission.object_box)
            obj_center_x = int((x1 + x2) / 2)
            obj_center_y = int((y1 + y2) / 2)
            current_depth = depth.get_value(obj_center_x, obj_center_y)[1]
            if current_depth > 5:
                cv2.putText(frame, "Special object too far (>5m), switching to rotate", (50, 480), FONT, 1, (0,0,255), 2)
                special_mission.phase = "rotate"
        else:
            if time.time() - special_mission.last_seen > 2:
                cv2.putText(frame, "Special object lost for 2+ sec, switching to rotate", (50, 480), FONT, 1, (0,0,255), 2)
                special_mission.phase = "rotate"
    ...
    # --- PHASE 2: APPROACH ---
        ...
        obj_center_x = int((x1 + x2) / 2)
        obj_center_y = int((y1 + y2) / 2)
        deviation = obj_center_x - center_x  # pozitif: ekranın sağında, negatif: solunda
        obj_depth = depth.get_value(obj_center_x, obj_center_y)[1]
        cv2.putText(frame, f"Special object depth: {obj_depth:.2f} m", (50, 310), FONT, 1, (0, 255, 0), 2)
        cv2.putText(frame, f"Special object center_x: {obj_center_x:} m", (50, 340), FONT, 1, (0, 255, 0), 2)


        if abs(deviation) > 40:
            cv2.putText(frame, "Centering special object...", (50, 280), FONT, 1, (255,255,0), 2)
            if deviation > 0:  # ekranın sağındaysa sola dön
                controller.set_servo(6, base_speed - abs(deviation)/8)
                controller.set_servo(5, base_speed + abs(deviation)/8)
            else:  # ekranın solundaysa sağa dön
                controller.set_servo(6, base_speed + abs(deviation)/8)
                controller.set_servo(5, base_speed - abs(deviation)/8)
            return False
        else:
            # Yaklaşma: nesne 1 metreye ulaşana kadar hareket
            if obj_depth > 1:
                controller.set_servo(5, base_speed+10)
                controller.set_servo(6, base_speed+10)
                return False
    ...
    # --- PHASE 3: MAINTAIN DISTANCE (1-1.5 m) FOR 5 SECONDS ---
        ...
        if abs(deviation) > 40:
            ...
            if obj_depth < 1.0:
                ...
                controller.set_servo(6, backwards+70)
                return False
            elif obj_depth > 1.5:
                controller.set_servo(5, base_speed)
                controller.set_servo(6, base_speed)
    ...
    # --- PHASE 4: SERVO HOLD ---
        ...
        if time.time() - special_mission.servo_hold_start >= 3:
    ...
    # --- PHASE 6: ROTATE TO ALIGN WITH RECORDED HEADING (±10°) ---
        ...
        if abs(heading_error) > 10:
            ...
            if hasattr(special_mission, "last_seen"):
                del special_mission.last_seen
            return True
...
def special_mission_triggered(frame, depth, center_x, center_y, cross_detected, triangle_detected, cross_positions, triangle_positions):
    if cross_detected:
        closest_depth = float('inf')
        closest_center = None
        # Tüm cross etrafına dikdörtgen çiziliyor:
        for box in cross_positions:
            x1, y1, x2, y2 = box
            cx = int((x1 + x2) / 2)
            cy = int((y1 + y2) / 2)
            depth_val = depth.get_value(cx, cy)[1]
...
def main():
    ...
    while True:
        ...
        if zed.grab() == sl.ERROR_CODE.SUCCESS:
            ...
            results = model(frame, conf=0.55)[0]
            ...
            for i, class_id in enumerate(class_ids):
                if class_id == 16 or class_id == 19:  # Kırmızı şamandıralar ve gateler 16: red_buoy 19: red_pole_buoy
                    red_detected = True
                    red_positions.append(coordinates[i])
                elif class_id == 23 or class_id == 24:  # Sarı şamandıralar 23: yellow_buoy 24: yellow_racquet_ball
                    yellow_detected = True
                    yellow_positions.append(coordinates[i])
                elif class_id == 11 or class_id == 13:  # Yeşil şamandıralar ve gateler 11: green_buoy 13: green_pole_buoy
                    green_detected = True
                    green_positions.append(coordinates[i])
                elif class_id == 4 or class_id == 7:  # Mavi şamandıra 4: blue_buoy 7: blue_racquet_ball
                    blue_detected = True
                    blue_positions.append(coordinates[i])
                elif class_id == 0:  # Siyah
                    black_detected = True
                    black_positions.append(coordinates[i])
                elif class_id == 2:  # siyah artı 2: black_cross      #todo: 1226
                    cross_detected = True
                    cross_positions.append(coordinates[i])
                elif class_id == 3:  # siyah üçgen 3: black_triangle
            ...
            if manual_mode:
                ...
                if special_mission_triggered(frame, depth, center_x, center_y, cross_detected, triangle_detected, cross_positions, triangle_positions):
