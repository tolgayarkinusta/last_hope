...
def main():
    ...
    # Main fonksiyonunun başlangıcında (diğer global değişkenlerle birlikte)
    ...
    while True:
        ...
        if zed.grab() == sl.ERROR_CODE.SUCCESS:
            ...
            if manual_mode:
                ...
                # Eğer herhangi bir tespit varsa son tespit zamanını güncelle
                if red_detected or green_detected or yellow_detected or blue_detected or black_detected:
                    last_detection_time = time.time()

                # Tespit durumu yoksa ve 15 saniye geçmişse 3 saniyelik süre kontrolü yap:
                else:
                    current_time = time.time()
                    if (current_time - start_time) >= 30 and (current_time - last_detection_time) >= 5 :
                        itsover_flag = True
                        # Eve dönüş algoritmasını çalıştır:
                        print("eve dönülüyor")
                ...
                # Otomatik modda, tüm renk fonksiyonları çalışır:
                    ...
                    if blue_detected and not black_detected:
                        #todo: blueOnly(blue_positions)
                        print("only blue") #top fırlatma
                    if not blue_detected and black_detected:
                        #todo: blackOnly()
                        print("only black") #su fışkırtma
                    if blue_detected and black_detected:
                        #todo: blueBlackOnly
